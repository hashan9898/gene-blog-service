...
public class MysqlDbBlogRepository implements BlogRepository {
    ...
    public Blog save(Blog blog, Boolean isCreate) throws SQLException {
        try {
            ...
            if (!isCreate) {
                ...
                blogDao.setCommentDaos(objectMapper.commentListToCommentDaoList(blog.getComments()));
            }
    ...
    @Transactional
    @Override
    public void deleteById(String blogId) throws SQLException {
        try {
            springDataMysqlBlogRepository.deleteById(blogId);
        } catch (Exception e) {
            throw new SQLException(e.getMessage(), e);
        }
    }
    ...
    public void saveDeleteComment(Blog blog, Boolean isCreated) throws SQLException {
        try {
            ...
            if (blogDao.isPresent()) {
                List<CommentDao> commentDaos = objectMapper.commentListToCommentDaoList(blog.getComments());
